...
class ForgotPasswordViewController: UIViewController {
    ...
    func validateFields() -> Bool {
        guard let email = tfPassword.text, !email.isEmpty else {
...
func call_WsForgotPassword(){
    ...
    dicrParam = ["email":self.tfPassword.text!]as [String:Any]
    ...
    objWebServiceManager.requestGet(strURL: WsUrl.url_ForgotPassword, params: dicrParam, queryParams: [:], strCustomValidation: "") { (response) in
        ...
        if status == MessageConstant.k_StatusCode{
            if let user_details  = response["result"] as? [String:Any] {
                ...
                objAlert.showAlertSingleButtonCallBack(alertBtn: "OK", title: "", message: "Reset link email sent to your registered email".localized(), controller: self) {
                    self.onBackPressed()
                }
